...
To do this, we can use `AWS IAM` service. `IAM` is `Identity and Access Management`.
...
Also select `"Autogenerated password"` option.

Make sure you check the option `"Users must create a new password at next sign-in"`.
...
**NOTE**: Copy the autogenerated password for the user.
...
Enter function name e.g. `myFirstLambdaFunction`.

Select the runtime e.g. `Python 3.8`

Finally click the button `"Create function"`.

In the function overview page, click the tab `"Configuration"` then click the link `"Triggers"`.

Then click the button `"Add trigger"`.

Select `"API Gateway"` from the `"Trigger configuration"` dropdown.

Then select the option `"Create a new API"`.
...
You should now have the `API endpoint` like below:

    https://6px2eh7aa7.execute-api.us-east-1.amazonaws.com/default/myFirstLambdaFunction

Now time to add code, click the tab `"Code"`.

You should see the autogenerated code like below:
...
You should make a note of two things, the file name and function name.

In the example above, the file name is `lambda_function.py` and function name is `lambda_handler`.

If you scroll down a bit and go to section `"Runtime settings"`. You should see the `Handler` looks like `lambda_function.lambda_handler`.

The format is `<filename>.<functionname>`.

Just to make it more descriptive, we will add one line to the function as below:
...
Enter the event name `MyHelloWorldEvent`.

Click the button `Save` and then button `Test`.
...
Click the tab `"Configuration"` then select the link `"Environment variables"` in the left hand side.

Click the button `Edit`.

Then click the button `"Add environment variable"`.

Enter `Key` and `Value`, for example: `API_USER` as key and `manwar` as value.

Then click the button `Save`.

Go back to code and change to print the value of environment variable `API_USER`.
...
Follow the guide to create keys [**here**](https://docs.aws.amazon.com/lambda/latest/dg/configuration-envvars.html).
